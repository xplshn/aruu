...
parseflag(int flag, char **args, char ***argv)
{
        ...
        switch (flag) {
        case 'j':
                /* ?man -j:jobs: number of parallel jobs (accepted, runs serially) */
        case 'f':
                /* ?man -f:file: read rules from file instead of makefile or Makefile */
                break;
        case 'e':
        ...
        case 'p':
                ...
                break;
        case 'q':
                /* ?man -q: question mode, return zero if up to date, one otherwise */
                qflag = 1;
                appendmakeflags("-q");
                break;
        case 'r':
