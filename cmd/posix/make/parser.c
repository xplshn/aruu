        ...
        *s = '\0';
        ...
        if (input->buf[0] == '.') {
                ...
                if (strncmp(d, "include", 7) == 0 && isblank(d[7])) {
                        input->pos = input->siz;
                        if (cond_active()) include(d + 7);
                        goto repeat;
                }
                if (strncmp(d, "sinclude", 8) == 0 && isblank(d[8])) {
                        input->pos = input->siz;
                        if (cond_active()) sinclude(d + 8);
                        goto repeat;
                }
        ...
        if (!strncmp(input->buf, "include", 7) && isblank(input->buf[7])) {
                input->pos = input->siz;
                if (cond_active()) include(input->buf+7);
                goto repeat;
        }
        ...
        if ((!strncmp(input->buf, "sinclude", 8) && isblank(input->buf[8])) ||
            (!strncmp(input->buf, "-include", 8) && isblank(input->buf[8]))) {
                input->pos = input->siz;
                if (cond_active()) sinclude(input->buf + 8);
                goto repeat;
        }
