...
static const struct t_op {
  ...
  opsm1[] =
      {
          {"r", FILRD},   {"w", FILWR},   {"x", FILEX},   {"e", FILEXIST}, {"f", FILREG},
          {"d", FILDIR},  {"c", FILCDEV}, {"b", FILBDEV}, {"p", FILFIFO},  {"u", FILSUID},
          {"g", FILSGID}, {"k", FILSTCK}, {"s", FILGZ},   {"t", FILTT},    {"z", STREZ},
          {"n", STRNZ},   {"h", FILSYM}, /* for backwards compat */
          {"O", FILUID},  {"G", FILGID},  {"L", FILSYM},  {"S", FILSOCK},  {"a", BAND},
          {"o", BOR},
},
  ops2[] =
      {
...
getn(const char *s)
{
  ...
  if (errno != 0)
    error((errno == EINVAL) ? "%s: bad number" : "%s: out of range", s);
...
equalf(const char *f1, const char *f2)
{
  ...
  return (
      stat(f1, &b1) == 0 && stat(f2, &b2) == 0 && b1.st_dev == b2.st_dev && b1.st_ino == b2.st_ino
  );
}
