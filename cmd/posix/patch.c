...
main(int argc, char *argv[])
{
  ...
  for (open_patch_file(filearg[1]); there_is_another_patch(); reinitialize_almost_everything()) {
    ...
    if (failed) {
      ...
      if (skip_rest_of_patch) {
        say("%d out of %d hunks ignored--saving "
            "rejects to %s\n",
            failed,
            hunk,
            rejname);
      } else {
        say("%d out of %d hunks FAILED -- saving "
            "rejects to %s\n",
            failed,
            hunk,
            rejname);
...
usage(void)
{
  fprintf(
      stderr,
      "usage: patch [-bCcEeflNnRstuv] [-B backup-prefix] [-D symbol] "
...
#define IN_ELSE   3
  ...
  while (old <= lastline) {
    if (pch_char(old) == '-') {
      ...
      say("Out-of-sync patch, lines %ld,%ld--mangled text or "
          "line numbers, maybe?\n",
          pch_hunk_beg() + old,
          pch_hunk_beg() + new);
      my_exit(2);
...
#define try(f, a1, a2, a3) (snprintf(lbuf, lbufsz, f, a1, a2, a3), stat(lbuf, &cstat) == 0)
    ...
    if (try("%s/RCS/%s%s", filedir, filebase, RCSSUFFIX)
      ...
      if (!statfailed) {
        if ((filestat.st_mode & 0222) != 0)
          fatal(
              "file %s seems to be locked by "
              "somebody else under RCS\n",
              filename
          );
        if (verbose)
          say("Comparing file %s to default RCS "
              "version...\n",
              filename);
        {
          char *rcsdiff_argv[3];
...
#undef try
  ...
  if (revision != NULL) {
    if (!rev_in_string(i_womp)) {
      if (force) {
        if (verbose)
          say("Warning: this file doesnt appear "
              "to be the %s version--patching "
              "anyway\n",
              revision);
      } else if (batch) {
        fatal(
            "this file doesnt appear to be the %s "
            "version--aborting\n",
            revision
        );
      } else {
        ask("This file doesnt appear to be the %s "
  ...
  if (using_plan_a)
    ...
    if (tiline[0] == baseline)
      ...
      if (lseek(tifd, (off_t)(baseline / lines_per_buf * BUFFERSIZE), SEEK_SET) < 0)
        pfatal(
            "cannot seek in the temporary input "
            "file"
        );
...
another_hunk(void)
{
  ...
  if (diff_type == CONTEXT_DIFF || diff_type == NEW_CONTEXT_DIFF) {
    ...
    while (p_end < p_max) {
      ...
      if (p_end >= hunkmax)
        fatal(
            "Internal error: hunk larger than hunk "
            "buffer size"
        );
      p_char[p_end] = *buf;
      p_line[p_end] = NULL;
      switch (*buf) {
        ...
        change_line:
          if (buf[1] == '\n' && canonicalize)
            strlcpy(buf + 1, " \n", bufsz - 1);
          if (!isspace((unsigned char)buf[1]) && buf[1] != '>' && buf[1] != '<' && repl_beginning
  ...
  hunk_done:
    ...
    if (*s != '@')
      malformed();
    if (p_first >= LINENUM_MAX - p_ptrn_lines || p_newfirst > LINENUM_MAX - p_repl_lines
...
backup_file(const char *orig)
{
  ...
  while (stat(bakname, &filestat) == 0 && orig_device == filestat.st_dev
         && orig_inode == filestat.st_ino) {
...
fetchname(const char *at, int *exists, int strip_leading)
{
  ...
  tab                 = strchr(t, '\t') != NULL;
  for (sleading = strip_leading; *t != '\0' && ((tab && *t != '\t') || !isspace((unsigned char)*t));
       t++) {
    if (t[0] == '/' && t[1] != '/' && t[1] != '\0') {
      if (--sleading >= 0)
        name = t + 1;
