...
usage(void)
{
  eprintf(
      ...
      "       sed [-nrE] -e script [-e script] ... [-f scriptfile] "
      "... [file ...]\n"
      "       sed [-nrE] [-e script] ... -f scriptfile [-f "
      "scriptfile] ... [file ...]\n"
  );
}
 ...
 * command) if delim is 0 all escaped characters represent themselves (aci text)
 * memmove rest of string (beyond end) into place
 * return the number of converted escapes (backslashes removed)
 * FIXME: this has had too many corner cases slapped on and is ugly. rewrite
 * better
 */
static size_t
  ...
  for (esc = 0, p = s; *p; p++) {
    if (esc) {
      ...
      if (isdigit(*p) && c->u.s.re && (size_t)(*p - '0') > c->u.s.re->re_nsub)
        leprintf(
            "back reference number greater than "
            "number of groups"
        );
    } else if (*p == '\\') {
      esc = 1;
