...
pax_pipe2(int fd[2], int flag)
{
  ...
  if (pipe(fd) != 0)
    return -1;
  if (flag & O_CLOEXEC
      && (fcntl(fd[0], F_SETFD, FD_CLOEXEC) != 0 || fcntl(fd[1], F_SETFD, FD_CLOEXEC) != 0))
    return -1;
  return 0;
}
#undef pipe2
...
readexthdr(struct bufio *f, struct header *h, off_t len)
{
  ...
  while (len > 0) {
    ...
    if (!end)
      fatal(
          "invalid extended header: record is missing "
          "newline"
      );
    *end   = '\0';
...
readcpio(struct bufio *f, struct header *h)
{
  ...
  switch (type) {
    ...
    default:
      fatal(
          "invalid cpio header: invalid or unsupported file type: "
          "%#o",
          type
      );
  }
    ...
    for (c = cmds; c < cmds + LEN(cmds); ++c) {
      if (l >= c->magiclen && memcmp(b, c->magic, c->magiclen) == 0) {
        if (lseek(f->fd, 0, SEEK_SET) != 0)
          fatal(
              "compression detection requires "
              "seekable input"
          );
        algo = c->algo;
        goto again;
...
writecpio(FILE *f, struct header *h)
{
  ...
  switch (h->type) {
    ...
    case CHRTYPE:
      ...
      break;
    default:
      fatal("unknown or unsupported header type");
  }
  if (h->dev > 0777777)
  ...
  len = snprintf(
      buf,
      sizeof buf,
      "070707%.6lo%.6lo%.6lo%.6lo%.6lo%.6lo%.6lo%.11llo%.6lo%.11jo",
      (unsigned long)h->dev,
      ino,
      mode,
      (unsigned long)h->uid,
      (unsigned long)h->gid,
      (unsigned long)h->nlink,
      (unsigned long)h->rdev,
      (unsigned long long)h->mtime.tv_sec,
      (unsigned long)namesize + 1,
      size
  );
  assert(len == 76);
  if (fwrite(buf, 1, 76, f) != 76)
...
parseopts(char *s)
{
  ...
  for (;;) {
    ...
    if (strcmp(key, "linkdata") == 0) {
      if (val)
        fatal(
            "option 'linkdata' must not have a "
            "value"
        );
      opt.linkdata = 1;
      ...
      if (strcmp(val, "bypass") != 0 && strcmp(val, "rename") != 0 && strcmp(val, "UTF-8") != 0
          && strcmp(val, "write") != 0) {
        fatal(
            "invalid action '%s' for option "
            "'invalid'",
            val
        );
      }
...
listhdr(FILE *f, struct header *h)
{
  ...
  timefmt = h->mtime.tv_sec + 15780000 < curtime || h->mtime.tv_sec > curtime ? "%b %e  %Y"
                                                                              : "%b %e %H:%M";
  ...
  printf(
      "%s %2ju %-8s %-8s %9s %s %s", mode, (uintmax_t)h->nlink, uname, gname, info, time, h->path
  );
  switch (h->type) {
    case LNKTYPE:
      printf(" == %s", h->link);
...
writefile(FILE *unused, struct header *h)
{
  ...
  if (uflag && fstatat(destfd, h->path, &st, 0) == 0) {
    if (h->mtime.tv_sec < st.st_mtime
        || (h->mtime.tv_sec == st.st_mtime && h->mtime.tv_nsec < st.st_mtim.tv_nsec))
      return;
  }
  if (vflag)
    fprintf(stderr, "%s\n", h->path);
