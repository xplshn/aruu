...
{
  eprintf(
      "usage: %s [-1 field] [-2 field] [-o list] [-e string] "
...
jlinecmp(struct jline *la, struct jline *lb, size_t jfa, size_t jfb)
{
  ...
  if (jfa >= la->nf) {
    ...
    status =
        memcmp(la->fields[jfa].s, lb->fields[jfb].s, MAX(la->fields[jfa].len, lb->fields[jfb].len));
    LIMIT(status, -1, 1);
  }
...
makespec(char *s)
{
  ...
  if (!strcmp(s, "0")) { /* join field must be 0 and nothing else */
    ...
    fileno = s[0] - '0';
    fldno =
        estrtonum(&s[2], 1, MIN((unsigned long long)LLONG_MAX, (unsigned long long)SIZE_MAX)) - 1;
  } else {
...
main(int argc, char *argv[])
{
  ...
  if (fshut(fp[0], argv[0]) | (fp[0] != fp[1] && fshut(fp[1], argv[1])) | fshut(stdout, "<stdout>"))
    ret = 2;
