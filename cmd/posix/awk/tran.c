...
Cell *
setsymtab(const char *n, const char *s, Awkfloat f, unsigned t, Array *tp)
{
  int   h;
  Cell *p;
  ...
  if (n != NULL && (p = lookup(n, tp)) != NULL) {
    DPRINTF(
        "setsymtab found %p: n=%s s=\"%s\" f=%g t=%o\n",
        (void *)p,
        NN(p->nval),
        NN(p->sval),
        p->fval,
  ...
  tp->tab[h] = p;
  DPRINTF(
      "setsymtab set %p: n=%s s=\"%s\" f=%g t=%o\n", (void *)p, p->nval, p->sval, p->fval, p->tval
  );
  return (p);
}
...
funnyvar(Cell *vp, const char *rw)
{
  ...
  WARNING(
      "funny variable %p: n=%s s=\"%s\" f=%g t=%o",
      (void *)vp,
      vp->nval,
      vp->sval,
      vp->fval,
...
#define update_str_val(vp)                                                                         \
  ...
  if (isstr(vp) == 0) {
    ...
    if (fmt == OFMT) {
      if ((vp->tval & CONVC) != 0 || ((vp->tval & CONVO) != 0 && vp->fmt != *fmt)) {
        update_str_val(vp);
        vp->tval &= ~CONVC;
        vp->tval |= CONVO;
        vp->fmt = *fmt;
      ...
      /* CONVFMT */
      if ((vp->tval & CONVO) != 0 || ((vp->tval & CONVC) != 0 && vp->fmt != *fmt)) {
        update_str_val(vp);
        vp->tval &= ~CONVO;
        vp->tval |= CONVC;
        vp->fmt = *fmt;
...
char *
tostringN(const char *s, size_t n) /* make a copy of string s */
{
  char *p;
...
Cell *
catstr(Cell *a, Cell *b) /* concatenate a and b */
{
  Cell  *c;
  char  *p;
  char  *sa = getsval(a);
