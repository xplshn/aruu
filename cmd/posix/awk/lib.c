...
{
  if ((record = (char *)malloc(n)) == NULL || (fields = (char *)malloc(n + 1)) == NULL
...
int
getrec(char **pbuf, int *pbufsize, bool isrecord) /* get next input record */
...
int
readcsvrec(char **pbuf, int *pbufsize, FILE *inf, bool newflag) /* csv can have \n's */
 ...
 * "..." */
  ...
  while ((c = getc(inf)) != EOF) {
    ...
    if (rr - buf + 1 > bufsize)
      if (!adjbuf(&buf, &bufsize, 1 + rr - buf, recsize, &rr, "readcsvrec 1"))
        FATAL("input record `%.30s...' too long", buf);
    *rr++ = c;
    if (c == '"')
      in_quote = !in_quote;
...
{                        /* nvals remain intact */
  ...
  for (i = n1; i <= n2; i++) {
    ...
    if (freeable(p))
      xfree(p->sval);
    p->sval = EMPTY, p->tval = FLD | STR | DONTFREE;
  }
}
...
int
refldbld(const char *rec, const char *fs) /* build fields from reg expr in FS */
{
  /* this relies on having fields[] the same length as $0 */
  /* the fields are all stored in this one array with \0's */
  char *fr;
...
bool
is_valid_number(const char *s, bool trailing_stuff_ok, bool *no_trailing, double *result)
{
  double r;
  char  *ep;
  bool   retval = false;
  ...
  if (s[0] == '+' || s[0] == '-') {
    ...
    if ((is_nan || is_inf) && (isspace((int)s[4]) || s[4] == '\0'))
      goto convert;
