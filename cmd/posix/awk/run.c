...
tempfree(Cell *p)
{
  if (p->ctype == OCELL && (p->csub < CUNK || p->csub > CFREE)) {
    WARNING("bad csub %d in Cell %d %s", p->csub, p->ctype, p->sval);
  }
  if (istemp(p))
    tfree(p);
}
...
{                        /* a[0] = BEGIN, a[1] = body, a[2] = END */
  ...
  if (a[0]) { /* BEGIN */
    ...
    if (isjump(x))
      FATAL(
          "illegal break, continue, next or nextfile from "
          "BEGIN"
      );
    tempfree(x);
  }
...
makearraystring(Node *p, const char *func)
{
  ...
  for (; p; p = p->nnext) {
    ...
    if (!adjbuf(&buf, &bufsz, tlen + 1, recsize, 0, func)) {
      FATAL("%s: out of memory %s[%s...]", func, x->nval, buf);
    }
...
u8_isutf(const char *s)
{
  ...
  } else if (n >= 3 && ((c >> 4) & 0xF) == 0xE && (s[1] & 0xC0) == 0x80 && (s[2] & 0xC0) == 0x80) {
    ret = 3; /* 1110xxxx 10xxxxxx 10xxxxxx */
  } else if (
      n >= 4 && ((c >> 3) & 0x1F) == 0x1E && (s[1] & 0xC0) == 0x80 && (s[2] & 0xC0) == 0x80
      && (s[3] & 0xC0) == 0x80
  ) {
...
int
u8_char2byte(const char *s, int charnum)
{
  int n;
  int bytenum = 0;
...
int
u8_byte2char(const char *s, int bytenum)
{
  int i, len, b;
...
int
format(char **pbuf, int *pbufsize, const char *s, Node *a) /* printf-like conversions */
{
  char       *fmt;
  char       *p, *t;
  const char *os;
  ...
  while (*s) {
    ...
    for (t = fmt; (*t++ = *s) != '\0'; s++) {
      if (!adjbuf(&fmt, &fmtsz, MAXNUMSIZE + 1 + t - fmt, recsize, &t, "format3"))
        FATAL(
            "format item %.30s... ran format() out "
            "of memory",
            os
        );
      ...
      if (*s == '*') {
        ...
        x = execute(a);
        a = a->nnext;
        snprintf(t - 1, FMTSZ(t - 1), "%d", fmtwd = (int)getfval(x));
        if (fmtwd < 0)
          fmtwd = -fmtwd;
    ...
    *t = '\0';
    ...
    switch (flag) {
      ...
      case 'A':
      case 'f':
        snprintf(p, BUFSZ(p), fmt, getfval(x));
        break;
      case 'd':
        snprintf(p, BUFSZ(p), fmt, (intmax_t)getfval(x));
        break;
      case 'u':
        snprintf(p, BUFSZ(p), fmt, (uintmax_t)getfval(x));
        break;
 ...
 * works */
        if (!has_utf8(t) || strcmp(fmt, "%s") == 0) {
          ...
          if (!adjbuf(&buf, &bufsize, 1 + n + p - buf, recsize, &p, "format7"))
            FATAL(
                "huge string/format (%d chars) "
                "in printf %.30s..."
                " ran format() out of memory",
                n,
                t
            );
...
static char *
nawk_convert(const char *s, int (*fun_c)(int), wint_t (*fun_wc)(wint_t))
{
  char        *buf  = NULL;
  char        *pbuf = NULL;
  const char  *ps   = NULL;
 ...
 */
    ...
    ps   = s;
    pbuf = buf;
    while (n = mbtowc(&wc, ps, sz), n > 0 && n != (size_t)-1 && n != (size_t)-2) {
      ps += n;
...
static char *
nawk_toupper(const char *s)
{
  return nawk_convert(s, toupper, towupper);
}

static char *
nawk_tolower(const char *s)
{
...
FILE *
openfile(int a, const char *us, bool *pnewflag)
{
  const char *s = us;
  size_t      i;
  int         m;
  ...
  for (i = 0; i < nfiles; i++)
    if (files[i].fname && strcmp(s, files[i].fname) == 0
        && (a == files[i].mode || (a == APPEND && files[i].mode == GT) || a == FFLUSH)) {
      if (pnewflag)
        *pnewflag = false;
      return files[i].fp;
    }
...
closefile(Node **a, int n)
{
  ...
  for (i = 0; i < nfiles; i++) {
    ...
    if (ferror(files[i].fp)) {
      if ((files[i].mode == GT && files[i].fp != stderr) || files[i].mode == '|')
        FATAL("write error on %s", files[i].fname);
      else
        WARNING("i/o error occurred on %s", files[i].fname);
    }
    if (files[i].fp == stdin || files[i].fp == stdout || files[i].fp == stderr)
...
Cell *
dosub(Node **a, int subop) /* sub and gsub */
{
  fa   *pfa;
  int   tempstat = 0;
  char *repl;
...
#define MT_REPLACE 2 /* selected, not empty */
    ...
    /* leading text: */
    if (patbeg > start) {
      adjbuf(&buf, &bufsz, (pb - buf) + (patbeg - start), recsize, &pb, "dosub");
      s = start;
      while (s < patbeg)
        *pb++ = *s++;
    }
    ...
    while (*r != 0) {
      ...
      if (*r == '\\') {
        ...
        r++;
        adjbuf(&buf, &bufsz, 1 + patlen + pb - buf, recsize, &pb, "dosub");
...
void
backsub(char **pb_ptr, const char **sptr_ptr) /* handle \\& variations */
{                                             /* sptr[0] == '\\' */
  char       *pb       = *pb_ptr;
...
static char *
wide_char_to_byte_str(int rune, size_t *outlen)
{
  static char buf[5];
  int         len;
