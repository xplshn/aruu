...
fa *
makedfa(const char *s, bool anchor) /* returns dfa for reg expr s */
{
  int        i, use, nuse;
  fa        *pfa;
  static int now = 1;
...
fa *
mkdfa(const char *s, bool anchor) /* does the real work of making a dfa */
...
int
quoted(const uschar **pp) /* pick up next thing after a \\ */
                          /* and increment *pp */
{
  const uschar *p = *pp;
  int           c;
 ...
 * be u8 size */
    ...
    if (i >= bufsz) {
      ...
      if (buf == NULL)
        FATAL(
            "out of space for character class "
            "[%.10s...] 2",
            p
        );
      bp = buf + i;
               ...
               /* returns 0 if p matches empty string */
{
  ...
  switch (type(p)) {
    ELEAF
    LEAF lp = info(p);                               /* look for high-water mark of subscripts */
...
int
member(int c, int *sarg) /* is c in s? */
{
  int *s = (int *)sarg;
...
static int
get_gototab(fa *f, int state, int ch) /* hide gototab implementation */
{
  gtte  key;
  gtte *item;
  ...
  item      = (gtte *)bsearch(
      &key, f->gototab[state].entries, f->gototab[state].inuse, sizeof(gtte), entry_cmp
  );
...
static int
entry_cmp(const void *l, const void *r)
{
...
static int
set_gototab(fa *f, int state, int ch, int val) /* hide gototab implementation */
{
  if ((unsigned)ch < GOTO_DIRECT) {
  ...
  qsort(f->gototab[state].entries, f->gototab[state].inuse, sizeof(gtte), entry_cmp);

  return val; /* not used anywhere at the moment */
}

static void
clear_gototab(fa *f, int state)
{
  memset(f->gototab[state].direct, 0, sizeof(f->gototab[state].direct));
  memset(f->gototab[state].entries, 0, f->gototab[state].allocated * sizeof(gtte));
...
bool
fnematch(fa *pfa, FILE *f, char **pbuf, int *pbufsize, int quantum)
{
  char *i, *j, *k, *buf = *pbuf;
 ...
 */
    if (k - j < (int)awk_mb_cur_max) {
      if (k + awk_mb_cur_max > buf + bufsize) {
        char *obuf = buf;
        adjbuf((char **)&buf, &bufsize, bufsize + awk_mb_cur_max, quantum, 0, "fnematch");
...
Node *
reparse(const char *p) /* parses regular expression pointed to by p */
...
concat(Node *np)
{
  switch (rtok) {
    ...
    case EMPTYRE:
      rtok = relex();
      return (concat(op2(CAT, op2(CCL, NIL, (Node *)cclenter("")), primary())));
  }
  return (np);
}
  ...
  if (secondnum < 0) { /* means {n,} -> repeat n-1 times followed by PLUS */
    if (firstnum < 2) {
      ...
      return replace_repeat(
          reptok, reptoklen, atom, atomlen, firstnum, secondnum, REPEAT_PLUS_APPENDED
      );
    }
 ...
 */
              ...
              FATAL(
                  "illegal repetition "
                  "expression: class %.20s",
                  lastre
              );
            }
          }
          if (repeat(starttok, prestr - starttok, lastatom, startreptok - lastatom, n, m) > 0) {
            if (n == 0 && m == 0) {
              return ZERO;
            }
 ...
 * and treat like just PLUS */
          ...
          FATAL(
              "illegal repetition expression: class "
              "%.20s",
              lastre
          );
        }
...
cgoto(fa *f, int s, int c)
{
  ...
  for (i = 1; i <= *p; i++) {
    if ((k = f->re[p[i]].ltype) != FINAL) {
      if ((k == CHAR && c == ptoi(f->re[p[i]].lval.np)) || (k == DOT && c != 0 && c != HAT)
          || (k == ALL && c != 0) || (k == EMPTYRE && c != 0)
