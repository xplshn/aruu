...
#define FORMAT_MISMATCHED(type) \
	...
	while ((ch = getopt_long(argc, argv, OPTIONS, longopts, NULL)) != -1) {
		...
		case OPT_TSIZE:
			...
			if (errstr) {
				warnx("invalid tabsize argument");
				usage();
			}
			break;
		case OPT_STRIPCR:
		...
		case OPT_COLOR:
			if (optarg == NULL || strncmp(optarg, "auto", 4) == 0)
				...
				errx(2, "unsupported color option %s", optarg);
			break;
	...
	if (gotstdin && (S_ISDIR(stb1.st_mode) || S_ISDIR(stb2.st_mode)))
		errx(2, "cant compare - to directory");
...
{
	errx(2, "conflicting output format options");
}
...
diffreg(char *file1, char *file2, int flags, int capsicum)
{
	...
	if (len[0] > INT_MAX - 2)
		errno = EFBIG, err(1, "%s", file1);
	if (len[1] > INT_MAX - 2)
		errno = EFBIG, err(1, "%s", file2);
...
opentemp(const char *f)
{
	...
	strlcpy(tempfile, _PATH_TMP "/diff.XXXXXXXX", sizeof(tempfile));
