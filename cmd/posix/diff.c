...
static struct Algorithm algorithms[] = {
    {"stone", D_DIFFSTONE}, {"myers", D_DIFFMYERS}, {"patience", D_DIFFPATIENCE}, {NULL, D_DIFFNONE}
};
...
// ?man synopsis: [-aBbdipTtw] [-c|-e|-f|-n|-q|-u|-y] [-A algo] [--brief]
...
// [--normal] [--rcs] [--show-c-function] [--starting-file]
// [--speed-large-files] [--strip-trailing-cr] [--tabsize number] [--text] [-I
// pattern] [-F pattern] [-L label] file1 file2
int
...
#define FORMAT_MISMATCHED(type) (diff_format != D_UNSET && diff_format != (type))
...
usage(void)
{
  fprintf(
      stderr,
      "usage: diff [-aBbdipTtw] [-c|-e|-f|-n|-q|-u|-y] [-A algo] "
  ...
  if (noderef) {
    ...
    if (S_ISLNK(stb1.st_mode) || S_ISLNK(stb2.st_mode)) {
      ...
      printf(
          ...
          path1,
          S_ISLNK(stb1.st_mode)   ? "symbolic link"
          : S_ISDIR(stb1.st_mode) ? "directory"
                                  : "file",
          path2,
          S_ISLNK(stb2.st_mode)   ? "symbolic link"
...
diffreg(char *file1, char *file2, int flags, int capsicum)
{
  ...
  if (diff_format == D_BRIEF && ignore_pats == NULL
      && (flags & (D_FOLDBLANKS | D_IGNOREBLANKS | D_IGNORECASE | D_SKIPBLANKLINES | D_STRIPCR))
             == 0) {
    rval = D_DIFFER;
    status |= 1;
    goto closem;
  ...
  if ((flags & D_FORCEASCII) != 0) {
    ...
    prepare(1, f2, stb2.st_size, flags);
      ...
      !asciifile(f1) || !asciifile(f2) || !prepare(0, f1, stb1.st_size, flags)
      || !prepare(1, f2, stb2.st_size, flags)
  ) {
    rval = D_BINARY;
    status |= 1;
...
prune(void)
{
  ...
  for (pref = 0;
       pref < len[0] && pref < len[1] && file[0][pref + 1].value == file[1][pref + 1].value;
       pref++)
    ;
  ...
  for (m /= 2; m != 0; m /= 2) {
    ...
    for (j = 1; j <= k; j++) {
      for (ai = &a[j]; ai > a; ai -= m) {
        ...
        if (aim < ai)
          break;
        if (aim->value > ai[0].value || (aim->value == ai[0].value && aim->serial > ai[0].serial))
          break;
        w.value      = ai[0].value;
...
fetch(long *f, int a, int b, FILE *lb, int ch, int oldfile, int flags)
{
  ...
  for (i = a; i <= b; i++) {
    ...
    if (diff_format != D_IFDEF && diff_format != D_GFORMAT && ch != '\0') {
      ...
      printf("%c", ch);
      if (Tflag
          && (diff_format == D_NORMAL || diff_format == D_CONTEXT || diff_format == D_UNIFIED))
        printf("\t");
      else if (diff_format != D_UNIFIED)
        printf(" ");
    }
    ...
    while (j < nc && (hw == 0 || col < hw)) {
      ...
      if (c == EOF) {
        if (diff_format == D_EDIT || diff_format == D_REVERSE || diff_format == D_NREVERSE)
  ...
  for (; pos > last; pos--) {
    ...
    if (most_recent_pat != NULL) {
      ...
      return lastbuf;
        ...
        isalpha(buf[0]) || buf[0] == '_' || buf[0] == '$' || buf[0] == '-' || buf[0] == '+'
    ) {
