...
static int
region_same(
    const struct difffile *a, struct region *ra, const struct difffile *b, struct region *rb
)
{
  size_t i;
  ...
  for (i = 0; i < ra->to - ra->from; i++) {
    if (a->lines[ra->from + i].len != b->lines[rb->from + i].len)
      return 0;
    if (memcmp(a->lines[ra->from + i].data, b->lines[rb->from + i].data, a->lines[ra->from + i].len)
        != 0)
      return 0;
  }
  return 1;
...
static int
merge_hunks(
    const struct difffile  *f1,
    const struct difffile  *f2,
    const struct difffile  *f3,
    const struct pair_hunk *d13,
    size_t                  n13,
    const struct pair_hunk *d23,
    size_t                  n23,
    struct three_hunk     **out,
    size_t                 *outn
)
{
...
main(int argc, char *argv[])
{
  ...
  if (diff_compute(&raw13, &f1, &f2, &opts) < 0 || diff_compute(&raw23, &f2, &f3, &opts) < 0) {
    diff_free(&f1);
    diff_free(&f2);
    diff_free(&f3);
    return 2;
