...
static int decode_cell(const char *s, const char **next, int *width,
                       int *is_underscore);
...
do_line(char *str, int reallydispl)
{
	...
	while (*str && pos > 0) {
		...
		if (*cursor == '\b' && *(cursor + 1) != '\0') {
			next_len = decode_cell(cursor + 1, &next, &tab_width,
			                       &next_is_underscore);
			if (!(is_underscore && *(cursor + 1 + next_len) != '\b')) {
				while (*cursor == '\b' && *(cursor + 1) != '\0') {
					cell_start = cursor + 1;
					cell_len = decode_cell(cell_start, &next, &cell_width,
					                       &is_underscore);
					cursor = next;
					do_hl = 1;
				}
