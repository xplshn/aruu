...
static struct val   *stack;
static jmp_buf       env;
static struct input *input;
static struct reg   *htbl[HASHSIZ];

static signed char onestr[] = {1, 0};
static struct num  zero, one = {.buf = onestr, .wp = onestr + 1};
...
/* this dc implementation follows the description of dc found in the paper
...
static struct num *
moreroom(struct num *num, int more)
{
  int          ro, wo, room;
  signed char *p;
...
newnum(int room)
{
  struct num *num = emalloc(sizeof(*num));
...
static struct num *
poke(struct num *num, unsigned d)
{
...
static int
begin(struct num *num)
{
  return num->begin != 0;
}

static int
first(struct num *num)
{
  num->begin = 0;
  num->rp    = num->buf;
...
static void
seek(struct num *num, int pos)
{
  num->rp = num->buf + pos;
}

static void
rshift(struct num *num, int n)
{
  int diff;
...
static struct num *
mulnto(struct num *src, struct num *dst, int n)
{
  div_t dd;
  int   d, carry;
...
static void
align(struct num *a, struct num *b)
{
  int         d;
  struct num *max, *min;
...
static struct num *
addnum(struct num *a, struct num *b)
{
  struct num *c;
...
static struct num *
subnum(struct num *a, struct num *b)
{
  struct num *tmp, *sum;
...
static struct num *
mulnum(struct num *a, struct num *b)
{
  struct num shadow, *c, *ca, *cb;
...
static struct num *
divmod(struct num *odivd, struct num *odivr, struct num **remp)
{
  struct num *acc, *divd, *divr, *res;
  int         divsign, remsign;
...
static int
divscale(struct num *divd, struct num *divr)
{
  int diff;
...
static struct num *
divnum(struct num *a, struct num *b)
{
  struct num *r;
  int         siga, sigb;
...
static struct num *
modnum(struct num *a, struct num *b)
{
  struct num *mod, *c;
  int         siga, sigb;
...
static struct num *
expnum(struct num *base, struct num *exp)
{
  int         neg, d;
  struct num *res, *fact, *e, *tmp1, *tmp2;
...
static int
cmpnum(struct num *a, struct num *b)
{
  struct num *diff;
  int         result;
...
static struct num *
sqrtnum(struct num *oy)
{
  struct num *y, *x, *xprev, *q, *sum;
  int         top, ysc, iter;
...
{
  char       *s, *t, *end, *dot;
  struct num *num, *denom, *numer, *frac, *q, *rem;
  int         sign, d, ch, nfrac;
...
static void
printnum(struct num *onum, int base)
{
  struct digit *sp;
  int           sc, i, sign, n;
  struct num   *num, *inte, *frac, *opow;
...
push(struct val v)
{
  struct val *p = emalloc(sizeof(struct val));
...
static void
pushnum(struct num *num)
{
  push((struct val){.type = NUM, .u.n = num});
}

static void
pushstr(char *s)
{
  push((struct val){.type = STR, .u.s = s});
}

static void
arith(struct num *(*fn)(struct num *, struct num *))
{
  struct num *a, *b, *c;
...
string(char *dst, int *np)
{
  ...
  for (;;) {
    ...
    switch (ch) {
      case '\0':
        /* the read above already stepped input->s one past this
         * buffer's own terminator; moreinput()'s own fast path
         * dereferences input->s directly on the assumption it is
         * always in bounds, so it has to be backed up first or that
         * check reads one byte past the allocation */
        --input->s;
        if (!moreinput())
          exit(0);
        break;
...
aryidx(void)
{
  ...
  if (n >= ap->n) {
    ap->buf = ereallocarray(ap->buf, n + 1, sizeof(struct val));
    for (i = ap->n; i <= n; ++i)
      ap->buf[i].type = NVAL;
    ap->n = n + 1;
  }
...
/* bc's own spawn() repurposes fd 0 as the read end of the pipe
 * carrying its generated code, and dups the real, original stdin to
 * fd 3 first so this command still has somewhere real to read from;
 * standalone dc (fd 3 never opened) falls back to its own real stdin */
static FILE *
readsrc(void)
{
  static FILE *fp;
...
eval(void)
{
  ...
  switch (ch) {
    ...
    case '=':
      ...
      break;
    case '?':
      /* real dc executes the line as commands rather than pushing it
       * as a string: bc's own read() relies on exactly this, since a
       * bare number is already a complete, valid command that pushes
       * itself */
      s = NULL;
      if (getline(&s, &siz, readsrc()) > 0) {
        addinput(NULL, s);
      } else {
        free(s);
        if (ferror(readsrc()))
...
// ?man dc: an arbitrary-precision reverse-polish desk calculator
// ?man arguments: [file ...]
// ?man dc reads and executes commands from each file in turn, then
// ?man from standard input
int
main(int argc, char *argv[])
{
  ARGBEGIN
  {
    // ?man -i: enable extended (multi-character) register names
    case 'i':
      iflag = 1;
      break;
    default:
