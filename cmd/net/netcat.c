...
main(int argc, char *argv[])
{
  ...
  if (lflag) {
    ...
    if (resolve(NULL, local_port, AF_UNSPEC, socktype, 1, &local_addr, &local_len) < 0)
      return 1;
    ...
    if (bind(listenfd, (struct sockaddr *)&local_addr, local_len) < 0)
      eprintf("bind:");

    if (!uflag) {
      if (listen(listenfd, 5) < 0)
        eprintf("listen:");
      sockfd = accept(listenfd, (struct sockaddr *)&remote_addr, &remote_len);
      if (sockfd < 0)
        eprintf("accept:");
      close(listenfd);
    } else {
      sockfd = listenfd;
      n      = recvfrom(
          sockfd, buf, sizeof(buf), MSG_PEEK, (struct sockaddr *)&remote_addr, &remote_len
      );
      if (n < 0)
        eprintf("recvfrom:");
      if (connect(sockfd, (struct sockaddr *)&remote_addr, remote_len) < 0)
        eprintf("connect:");
    }
  } else {
    /* client mode */
    ...
    if (resolve(host, port, AF_UNSPEC, socktype, 0, &remote_addr, &remote_len) < 0)
      return 1;
    ...
    if (local_port) {
      memset(&local_addr, 0, sizeof(local_addr));
      if (resolve(NULL, local_port, remote_addr.ss_family, socktype, 1, &local_addr, &local_len)
          < 0)
        return 1;
      opt = 1;
