...
display_interface(const struct NetInterface *iface)
{
  ...
  if (iface->has_mac) {
    printf(
        "Ethernet  HWaddr %02x:%02x:%02x:%02x:%02x:%02x\n",
        iface->mac[0],
        iface->mac[1],
        iface->mac[2],
        iface->mac[3],
  ...
  if (net_get_stats(iface->name, &stats) >= 0) {
    printf(
        "        RX packets:%llu errors:%llu dropped:%llu "
        "overruns:0 frame:0\n",
        stats.rx_packets,
        stats.rx_errs,
        stats.rx_drop
    ...
    printf(
        "        TX packets:%llu errors:%llu dropped:%llu "
  ...
  while (i < argc) {
    ...
    if (strcmp(arg, "up") == 0) {
      if (ioctl(sock, SIOCGIFFLAGS, &ifr) < 0)
        eprintf("ioctl SIOCGIFFLAGS:");
      ifr.ifr_flags |= IFF_UP | IFF_RUNNING;
      if (ioctl(sock, SIOCSIFFLAGS, &ifr) < 0)
        eprintf("ioctl SIOCSIFFLAGS:");
      i++;
      ...
      if (i + 1 >= argc)
        eprintf("%s needs an address\n", arg);
      sin             = (struct sockaddr_in *)&ifr.ifr_dstaddr;
      sin->sin_family = AF_INET;
      if (inet_pton(AF_INET, argv[i + 1], &sin->sin_addr) <= 0)
        eprintf("invalid address: %s\n", argv[i + 1]);
      if (ioctl(sock, SIOCSIFDSTADDR, &ifr) < 0)
        eprintf("ioctl SIOCSIFDSTADDR:");
      if (ioctl(sock, SIOCGIFFLAGS, &ifr) < 0)
        eprintf("ioctl SIOCGIFFLAGS:");
      ...
      if (inet_pton(AF_INET, arg, &sin->sin_addr) <= 0) {
        eprintf(
            "unknown action or invalid address: "
            "%s\n",
            arg
        );
      }
      if (ioctl(sock, SIOCSIFADDR, &ifr) < 0)
        eprintf("ioctl SIOCSIFADDR:");
