...
main(int argc, char *argv[])
{
  ...
  for (; *argv; argc--, argv++) {
#if FEATURE_NOEXEC
    /* builtins are function pointers with no filesystem path */
    if (wexec_get_noexec() && wexec_is_builtin(*argv)) {
      printf("%s: builtin applet\n", *argv);
      found = 1;
      if (!aflag)
        continue;
    }
...
#if FEATURE_NOEXEC
      /* already reported above when builtin was active */
      if (!wexec_get_noexec() || !wexec_is_builtin(*argv))
#endif
      {
