...
is_excluded(const char *path)
{
  ...
  for (i = 0; i < excludes_cnt; i++) {
    if (fnmatch(excludes[i], path, 0) == 0 || fnmatch(excludes[i], base, 0) == 0)
      return 1;
  }
  return 0;
}
...
comp(int fd, const char *tool, const char *flags)
{
  ...
  switch (fork()) {
    ...
    case 0:
      ...
      {
        char *tar_argv[3];
        tar_argv[0] = (char *)tool;
        tar_argv[1] = (char *)flags;
        tar_argv[2] = NULL;
        wexecvp_self(tool, tar_argv);
      }
      weprintf("wexecvp %s:", tool);
      _exit(1);
  }
  close(fds[0]);
  return fds[1];
...
sanitize(struct header *h)
{
  ...
  for (i = 0; i < LEN(fields); i++) {
    ...
    if (fields[i].f[l])
      eprintf(
          "numeric field #%d (%.*s) is not null or space "
          "terminated\n",
          i,
          l + 1,
          fields[i].f
...
#if FEATURE_TAR_CREATE
  eprintf(
      "usage: %s [x | t | -x | -t] [-C dir] [-J | -Z | -a | -j | -z] "
      "[-m] [-p] "
      "[-f file] [file ...]\n"
      "       %s [c | -c] [-C dir] [-J | -Z | -a | -j | -z] [-h] "
      "path ... "
      "[-f file]\n",
      argv0,
      argv0
...
// ?man tar [c | -c] [-C dir] [-J | -Z | -a | -j | -z] [-h] [-T file] [-X file]
// path ... [-f file] ?man manipulate tape archive files
int
