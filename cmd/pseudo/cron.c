...
struct field {
  enum { ERROR, WILDCARD, NUMBER, RANGE, REPEAT, LIST } type;
  long *val;
  int   len;
};
...
waitjob(void)
{
  ...
  while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
    ...
    if (WIFEXITED(status) == 1)
      ...
      loginfo(
          "complete: pid: %d terminated by signal: %s "
          "time: %s",
          pid,
      ...
      loginfo(
          "complete: pid: %d stopped by signal: %s time: "
          "%s",
          pid,
  ...
  for (y = 0; (len = getline(&line, &size, fp)) != -1; y++) {
    ...
    for (x = 0; x < LEN(flim); x++) {
      ...
      if (!col || parsefield(col, flim[x].min, flim[x].max, flim[x].f) < 0) {
        logerr(
            "error: failed to parse `%s' field on "
            "line %d\n",
            flim[x].name,
            y + 1
        );
