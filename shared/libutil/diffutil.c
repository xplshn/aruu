...
static int
lines_equal(const struct diffline *a, const struct diffline *b, const struct diffopts *opts)
{
  size_t i, j;
  ...
  if (opts && opts->ignore_blanks) {
    ...
    while (i < a->len && j < b->len) {
      ...
      if (i >= a->len || j >= b->len)
        break;
      if (opts->ignore_case) {
        if (tolower((unsigned char)a->data[i]) != tolower((unsigned char)b->data[j]))
          return 0;
...
int
diff_compute(
    struct diffhunks      *hunks,
    const struct difffile *old,
    const struct difffile *new,
...
)
{
  ...
  for (i = 0; i <= n; i++) {
    for (j = 0; j <= m; j++) {
      ...
      if (i > 0 && j > 0 && lines_equal(&old->lines[i - 1], &new->lines[j - 1], opts)) {
        c[i * (m + 1) + j].diag = c[(i - 1) * (m + 1) + (j - 1)].diag + 1;
        c[i * (m + 1) + j].up   = 0;
        c[i * (m + 1) + j].left = 0;
        continue;
      }
      if (i > 0 && (j == 0 || c[(i - 1) * (m + 1) + j].diag >= c[i * (m + 1) + (j - 1)].diag)) {
...
static void
print_lines(FILE *out, const struct diffline *lines, size_t from, size_t to, char prefix)
{
  size_t i;
  for (i = from; i < to; i++) {
    fputc(prefix, out);
...
static int
format_normal(
    FILE *out, const struct diffhunks *hunks, const struct difffile *old, const struct difffile *new
)
{
  size_t i;
  for (i = 0; i < hunks->n; i++) {
