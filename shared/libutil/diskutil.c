...
blockdev_get_list(void)
{
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		snprintf(info->path, sizeof(info->path), "%s/%s", PATH_DEV, name);
		info->size = blocks * 1024;
		info->major = major;
		info->minor = minor;

		/* search tree topology to link partition node under proper target parent disk */
		is_part = 0;
	...
	mfile = fopen(PATH_MOUNTS, "r");
	if (mfile) {
		while (fgets(mline, sizeof(mline), mfile)) {
			if (sscanf(mline, "%127s %255s %63s", devpath, mnt, fs) == 3) {
				char *devname = devpath;
				size_t dev_len = strlen(PATH_DEV);
				if (strncmp(devpath, PATH_DEV, dev_len) == 0 && devpath[dev_len] == '/')
	...
	while ((tok = strsep(&ptr, " \t")) != NULL) {
		...
		snprintf(info->path, sizeof(info->path), "%s/%s", PATH_DEV, tok);
		strlcpy(info->type, "disk", sizeof(info->type));

		snprintf(devpath, sizeof(devpath), "%s/%s", PATH_DEV, tok);
			...
			while ((de = readdir(dir)) != NULL) {
				if (strncmp(de->d_name, tok, disk_len) == 0) {
					...
					if (c == 's' || c == 'p') {
						...
						snprintf(part->path, sizeof(part->path), "%s/%s", PATH_DEV, de->d_name);
						strlcpy(part->type, "part", sizeof(part->type));
						snprintf(devpath, sizeof(devpath), "%s/%s", PATH_DEV, de->d_name);
						if (blockdev_open(&dev, devpath, 0) == 0) {
							part->size = dev.size;
							blockdev_close(&dev);
						}
	...
	for (i = 0; i < mntsize; i++) {
		...
		size_t dev_len = strlen(PATH_DEV);
		if (strncmp(frompath, PATH_DEV, dev_len) == 0 && frompath[dev_len] == '/')
	...
	while ((tok = strsep(&ptr, ",")) != NULL) {
		...
		strlcpy(info->name, tok, sizeof(info->name));
		snprintf(info->path, sizeof(info->path), "%s/r%sc", PATH_DEV, tok);
		strlcpy(info->type, "disk", sizeof(info->type));

		snprintf(devpath, sizeof(devpath), "%s/r%sc", PATH_DEV, tok);
		if (blockdev_open(&dev, devpath, 0) == 0) {
			info->size = dev.size;
			blockdev_close(&dev);
		}
		...
		for (c = 'a'; c <= 'p'; c++) {
			if (c == 'c')
				continue;
			snprintf(devpath, sizeof(devpath), "%s/r%s%c", PATH_DEV, tok, c);
			if (blockdev_open(&dev, devpath, 0) == 0) {
				part = ecalloc(1, sizeof(*part));
				snprintf(part->name, sizeof(part->name), "%s%c", tok, c);
				snprintf(part->path, sizeof(part->path), "%s/r%s%c", PATH_DEV, tok, c);
				strlcpy(part->type, "part", sizeof(part->type));
				part->size = dev.size;
				p = info->parts;
				if (!p) {
	...
	for (c = 'a'; c <= 'z'; c++) {
		...
		snprintf(devpath, sizeof(devpath), "%s/%s", PATH_DEV, devname);
		if (blockdev_open(&dev, devpath, 0) == 0) {
			info = ecalloc(1, sizeof(*info));
			strlcpy(info->name, devname, sizeof(info->name));
			snprintf(info->path, sizeof(info->path), "%s/%s", PATH_DEV, devname);
			strlcpy(info->type, "disk", sizeof(info->type));
			info->size = dev.size;
			if (!head) {
				head = info;
...
zfs_detect(const unsigned char *buf, size_t len)
{
	...
	for (offset = 131072; offset + 8 <= len; offset += 1024) {
		...
		if (magic == 0x00bab10c00000000ULL || magic == 0x0cb1ba0000000000ULL ||
...
plan9_detect_text(const unsigned char *buf, size_t len, const char **name)
{
	...
	while (i < len) {
		...
		if (sscanf(line, "%63s %d", key, &val) == 2) {
			if (strcmp(key, "blocksize") == 0) blocksize = val;
			else if (strcmp(key, "namelen") == 0) namelen = val;
			else if (strcmp(key, "dirblks") == 0) dirblks = val;
			else if (strcmp(key, "indirblks") == 0) indirblks = val;
			else if (strcmp(key, "daddrbits") == 0) daddrbits = val;
		}
	}

	for (j = 0; j < LEN(p9_fs_defs); j++) {
		if (blocksize == p9_fs_defs[j].blocksize &&
		    namelen == p9_fs_defs[j].namelen &&
		    dirblks == p9_fs_defs[j].dirblks &&
		    indirblks == p9_fs_defs[j].indirblks &&
...
int
blockdev_detect_fs(struct BlockDev *dev, char *type, size_t type_sz,
                   char *label, size_t label_sz, char *uuid, size_t uuid_sz)
{
	unsigned char *buf;
